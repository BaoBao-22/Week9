...
public class Bank {
  ...
  /**
   * Main method to demonstrate the banking system with professional logging.
   *
   * @param args command line arguments
   */
  public static void main(String[] args) {
    logger.info("Bank System Application is starting...");
    
    Bank bank = new Bank();
    Customer customer = new Customer(123456789L, "Nguyen Van A");
    bank.getCustomerList().add(customer);
    
    logger.info("Initialized bank with {} customers", bank.getCustomerList().size());
    
    customer.addAccount(new CheckingAccount(1001, 500.0));
    logger.info("Added CheckingAccount to customer: {}", customer.getFullName());
    
    logger.info("Bank System Application finished successfully.");
  }
}